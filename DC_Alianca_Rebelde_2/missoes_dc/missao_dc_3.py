...
# ==============================================================================
# FUNÇÕES AUXILIARES DO ALGORITMO (Sem alterações)
# ==============================================================================
...
# ==============================================================================
# ### MUDANÇA ### - CLASSE DO DROIDE DE AUXÍLIO
# A classe agora herda de tk.LabelFrame para ser encaixada no painel lateral.
# ==============================================================================

class HelperDroid(tk.LabelFrame):
    def __init__(self, master, game_manager):
        # Tenta obter cores e fontes do game_manager
        try:
            bg_color = game_manager.cor_fundo
            fg_color = game_manager.fg_color_light
            title_color = "cyan"
            droid_font = game_manager.narrative_font_obj
            title_font = game_manager.small_bold_font_obj
        ...
        # Configura o próprio LabelFrame
        super().__init__(master, text=" ● PA09DC - Tactical Droid ",
                         font=title_font, bg=bg_color, fg=title_color,
                         padx=10, pady=10)

        # Rótulo interno para exibir as mensagens do droide
        self.message_label = tk.Label(
            self, text="Aguardando comandos...",
            font=droid_font,
            wraplength=260,  # Ajustado para o painel lateral
            justify=tk.LEFT,
            fg=fg_color,
            bg=bg_color
        )
    ...
    def show_message(self, message):
        """Atualiza o texto da mensagem no painel."""
        self.message_label.config(text=message)

    def hide(self):
        """"Esconde" o droide limpando a mensagem."""
        self.show_message("...")
...
class MissaoDC3:
    def __init__(self, root, game_manager, content_frame_para_missao):
        ...
        # ### MUDANÇA ### - Droide será inicializado depois que o painel for criado
        self.helper_droid = None
    ...
    def _limpar_frame_e_eventos(self):
        # ### MUDANÇA ### - Adiciona verificação para o caso do droide não existir ainda
        if self.helper_droid:
    ...
    def iniciar_fase_interativa(self):
        ...
        # ### MUDANÇA ### - Layout do Log e do Droid
        # O Log agora tem uma altura definida e não expande infinitamente.
        # O Droid é instanciado e posicionado abaixo do log.
        ...
        log_frame.pack(pady=(0, 10), fill=tk.X, expand=False)
        
        self.log_listbox = tk.Listbox(log_frame, font=("Courier", 9), bg="#1c1c1c", fg="lightgrey", borderwidth=0, highlightthickness=0, height=12) # Altura fixa
        self.log_listbox.pack(fill=tk.BOTH, expand=True)

        # Instancia e posiciona o HelperDroid no painel
        self.helper_droid = HelperDroid(control_panel, self.game_manager)
        self.helper_droid.pack(fill=tk.BOTH, expand=True)
    ...
    def _atualizar_guia(self, proximo_estado=None):
        ...
        # ### MUDANÇA ### - Chamada do método show_message simplificada
        if mensagem:
             self.helper_droid.show_message(mensagem)

    # O restante do código da classe MissaoDC3 (a partir de _setup_inicial_canvas)
    # permanece o mesmo e não precisa ser alterado.
    # ... (cole o restante do seu código original aqui, sem modificações)
